...
open class BaseKlippyService(private val num: Int) : BasePythonService() {
    ...
    override fun onStartPython() {
        ...
        try {
            ...
            if (engineDirName == Prefs.ENGINE_KALICO) {
                Log.i("klippy_$num", "Kalico engine requires Python stdlib 'grp' which is unavailable on Android (Chaquopy). Forcing Klipper engine.")
                engineDirName = Prefs.ENGINE_KLIPPER
            }
